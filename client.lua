function crnoConvertLuaTextIntoHtml(text)
...
function ShowNotification(message, timeout)
    SendNUIMessage({
        ...
        mapaopen = IsRadarEnabled(),
        message = crnoConvertLuaTextIntoHtml(message),
        color = crnoGetColor(message),
        id = math.random(1, 5000000),
        id2 = math.random(1, 50000000)
    })
end

RegisterNetEvent('crno_notifications:showNotification')
AddEventHandler('crno_notifications:showNotification', function(message, timeout)
    ShowNotification(message, timeout)
end)

RegisterNetEvent('crno_notifications:deleteNotis')
AddEventHandler('crno_notifications:deleteNotis', function()
    SendNUIMessage({
        delete = true,
    })
end)
